...
const addValues = (arr) => {
  return arr.reduce((sum, num, idx) => { sum += num; return sum;}, 0);
};
...
const addPurchases = (arr) => {
  return arr.reduce((totalPurchases, obj, idx) => {
    totalPurchases += obj.purchasePrice;
...
const returnNames = (arr) => {
  return arr.reduce((acc, person) => [...acc, person.name], []); // essentially, place a person.name inside of the account array with every person.name that exists.
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((totalNumOfChildren, fam) => totalNumOfChildren + fam.children, 0);
};
