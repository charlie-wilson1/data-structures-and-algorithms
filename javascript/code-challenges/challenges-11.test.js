...
function transformToLis(obj) {
  ...
  let ans = keys.map((key, idx) => {
    return `<li>${key}: ${values[idx]}</li>`;
  });
  return ans;
}
...
const totalSum = (input) => {
  let sum = input.reduce((totalSum, currentArr) => {
    totalSum += currentArr.reduce((innerSum, currentVal) => {
      innerSum += currentVal;
      return innerSum;
    }), 0;
    return totalSum;
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(innerArr => {
    let filteredArr = innerArr.filter(val => val % 5 === 0 && typeof val === 'number');
    return filteredArr.map(filteredVal => {
      return Math.pow(2, filteredVal);
    });
  });
};
...
let findShortest = (data) => {
  let shortest = 9999;
  let name = data.reduce((name, currentObj) => {
    if (parseInt(currentObj.height) < shortest) {
      shortest = parseInt(currentObj.height);
