...
const longestString = (arr) => {
  let longestStr = '';
  arr.forEach(str => str.length > longestStr.length ? longestStr = str : str);
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(str => {
    ...
    let reduceArr = [];
    splitArr.forEach(c => {
      /[0-9]/.test(c) ? reduceArr.push(c) : c;
    });
    let finalStr = reduceArr.join('');
    return finalStr;
  });
};
/* ------------------------------------------------------------------------------------------------
...
const onlyOddChars = (str) => {
  ...
  splitStr.forEach(c => splitStr.indexOf(c) % 2 === 0 ? c : ansArr.push(c));
  let finalStr = ansArr.join('');
  return finalStr;
};
